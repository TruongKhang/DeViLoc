...
from deviloc.utils.read_write_model import read_model, write_model
...
from .cambridge import create_query_list_with_intrinsics
...
def create_reference_sfm(full_model, ref_model, blacklist=None, ext='.bin'):
    ...
    This function is ported from hloc/pipelines/7Scenes/utils.py'''
    ...
    print(f'Kept {len(images_ref)} images out of {len(images)}.')


class Se7Scenes(Dataset):
    def __init__(self, root_dir, scene_name, pair_file,
        ...
        self.colmap_dir = f"{root_dir}/7scenes_sfm_triangulated/{scene_name}/triangulated"
        self.image_dir = f"{root_dir}/{scene_name}"

        self.query_file_with_intrinsics = f"{self.colmap_dir}/query_list_with_intrinsics.txt"
        create_query_list_with_intrinsics(Path(f"{self.colmap_dir}"), self.query_file_with_intrinsics, 
                                          f"{self.colmap_dir}/list_test.txt")
        self.query_pairs = self.load_query_info(f"{pair_file}", self.query_file_with_intrinsics)
        ...
        ref_sfm_model = f"{self.colmap_dir}/ref_sfm_model"
        create_reference_sfm(Path(f"{self.colmap_dir}"), Path(ref_sfm_model), Path(f"{self.colmap_dir}/list_test.txt"))
        self.db_img_info = self.load_sfm_model(ref_sfm_model)
    ...
    def load_sfm_model(self, sfm_model_dir, file_type=".bin"):
        ...
        # pts3d = {
        #     v.id: v.xyz for v in pts3d.values()
        # }
